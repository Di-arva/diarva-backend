...
async function listApplicationsForTask(taskId, clinicId) {
    ...
    if (!task) {
        throw new Error("Task not found or does not belong to the specified clinic.");
    }
    return Application.find({ task_id: taskId }).populate("applicant_id", "first_name last_name email mobile");
}
...
async function acceptApplication(applicationId, clinicId, adminId) {
    ...
    try {
        const application = await Application.findById(applicationId).session(session).populate('applicant_id');
        ...
        if (task.status !== 'open') throw new Error("This task is no longer open for applications.");
        ...
        task.assignment = { assigned_to: application.applicant_id._id, assigned_at: new Date() };
        await task.save({ session });

        // 3. Reject all other pending applications for this task
        const otherApplications = await Application.find({ task_id: task._id, status: 'pending' }).session(session).populate('applicant_id');
        await Application.updateMany(
            { task_id: task._id, status: 'pending' },
            { $set: { status: 'rejected', reviewed_at: new Date(), reviewed_by: adminId } },
            { session }
        );
        ...
        }).catch(err => logger.error(`Failed to send acceptance email to ${application.applicant_id.email}`, err));
        ...
        otherApplications.forEach(app => {
            ...
            }).catch(err => logger.error(`Failed to send rejection email to ${app.applicant_id.email}`, err));
        });
...
async function rejectApplication(applicationId, clinicId) {
    const application = await Application.findById(applicationId).populate('applicant_id');
    if (!application) throw new Error("Application not found.");
    ...
    }).catch(err => logger.error(`Failed to send rejection email to ${application.applicant_id.email}`, err));
...
async function withdrawApplication(applicationId, applicantId) {
    ...
    session.startTransaction();
    try {
        const application = await Application.findOne({ _id: applicationId, applicant_id: applicantId }).session(session);
        if (!application) throw new Error("Application not found or you do not have permission to withdraw it.");

        if (!['pending', 'under_review'].includes(application.status)) {
            throw new Error(`Application cannot be withdrawn because its status is '${application.status}'.`);
        }

        application.status = 'withdrawn';
        await application.save({ session });
        
        await Task.findByIdAndUpdate(application.task_id, { $inc: { applications_count: -1 } }, { session });
        
        await session.commitTransaction();
        
        // Optional: Notify clinic of withdrawal
        logger.info(`Application ${applicationId} withdrawn by user ${applicantId}.`);
...
async function getAssistantContext(userId, log) {
  ...
  log.info({ msg: "assistant context fetched", duration_ms: Date.now() - t0, hasUser: !!user, hasProfile: !!profile });

  if (!user || user.role !== "assistant" || user.approval_status !== "approved" || !user.is_active) {
    return { ok: false, status: 403, message: "Assistant is not active or approved" };
  }
...
async function discoverForAssistant(userId, opts, ctx = {}) {
  const log = logger.child({ reqId: ctx.reqId || "n/a", actor: ctx.actor || "n/a", userId });

  const context = await getAssistantContext(userId, log);
  if (!context.ok) return { total: 0, page: 1, pages: 1, limit: opts?.limit || 20, data: [] };

  const { cert, specs } = context;
  const { page = 1, limit = 20, sort = { "schedule.start_datetime": 1 }, filters = {} } = opts || {};
  ...
  if (cert && CERT_ENUM.includes(cert)) certClause.push({ "requirements.certification_level": cert });
  query.$or = certClause;
  ...
  if (!filters["requirements.required_specializations"] && Array.isArray(specs) && specs.length > 0) {
    query.$and = [
      {
        $or: [
  ...
  query["schedule.start_datetime"].$gte = query["schedule.start_datetime"].$gte || now;
  query.$and = query.$and || [];
  query.$and.push({
    $or: [{ application_deadline: { $exists: false } }, { application_deadline: { $gte: now } }],
  });
  ...
  log.info({ msg: "db.count Task discover", duration_ms: Date.now() - tCount, total });
  ...
  const items = await Task.find(query)
    ...
    .lean();
  log.info({ msg: "db.find Task discover", duration_ms: Date.now() - tList, count: items.length });
...
async function apply(userId, taskId, ctx = {}) {
  const log = logger.child({ reqId: ctx.reqId || "n/a", actor: ctx.actor || "n/a", userId, taskId });

  // Ensure assistant is active/approved
  const context = await getAssistantContext(userId, log);
  if (!context.ok) return { ok: false, status: context.status, message: context.message, reason: "assistant_not_active" };
  ...
  try {
    ...
    log.info({ msg: "db.findOne Task for apply", duration_ms: Date.now() - t0, found: !!task });

    if (!task) {
      await session.abortTransaction();
      session.endSession();
      return { ok: false, status: 404, message: "Task not available", reason: "task_not_available" };
    }
    ...
    log.info({ msg: "db.findOne Application duplicate check", duration_ms: Date.now() - t1, found: !!dup });

    if (dup) {
      await session.abortTransaction();
      session.endSession();
      return { ok: false, status: 409, message: "Already applied for this task", reason: "duplicate_application" };
    }
    ...
    log.info({ msg: "db.insert Application", duration_ms: Date.now() - t2, applicationId: appDoc._id });
    ...
    log.info({ msg: "db.updateOne Task inc applications_count", duration_ms: Date.now() - t3, matched: incRes.matchedCount, modified: incRes.modifiedCount });

    if (incRes.modifiedCount === 0) {
      await session.abortTransaction();
      session.endSession();
      return { ok: false, status: 409, message: "Task application limit reached", reason: "limit_reached" };
    }
    ...
    await session.abortTransaction();
    session.endSession();
    log.error({ msg: "assistantTaskService.apply error", error: err.message, stack: err.stack });
    throw err;
  }
}
