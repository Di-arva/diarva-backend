...
const create = async (req, res, next) => {
  ...
  const user = req.user || {};
  log.info({
    msg: "clinicTaskController.create called",
    userId: user.sub,
    role: user.role,
  });
  ...
  try {
    ...
    //   errors.push(
    //     `requirements.certification_level must be one of: ${CERT_ENUM.join(
    //       ", "
    //     )}`
    //   );
    // }
    ...
    if (!schedule.start_datetime)
      errors.push("schedule.start_datetime is required");
    if (!schedule.end_datetime)
      errors.push("schedule.end_datetime is required");

    let start = schedule.start_datetime
      ? parseDate(schedule.start_datetime)
      : null;
    let end = schedule.end_datetime ? parseDate(schedule.end_datetime) : null;
    if (start && isNaN(start.getTime()))
      errors.push("schedule.start_datetime must be a valid date");
    if (end && isNaN(end.getTime()))
      errors.push("schedule.end_datetime must be a valid date");
    if (start && end && end <= start)
      errors.push(
        "schedule.end_datetime must be after schedule.start_datetime"
      );
    ...
    if (comp.payment_method && !PAY_METHOD_ENUM.includes(comp.payment_method)) {
      errors.push(
        `compensation.payment_method must be one of: ${PAY_METHOD_ENUM.join(
          ", "
        )}`
      );
    }
    if (comp.payment_terms && !PAY_TERMS_ENUM.includes(comp.payment_terms)) {
      errors.push(
        `compensation.payment_terms must be one of: ${PAY_TERMS_ENUM.join(
          ", "
        )}`
      );
    }
    ...
    if (start && end) {
      ...
      if (durationHours < 0.5 || durationHours > 12) {
        errors.push(
          "Computed duration_hours must be between 0.5 and 12 (consider adjusting break or times)"
        );
      }
    }
    ...
    if (errors.length) {
      log.warn({
        msg: "clinicTaskController.create validation failed",
        errors,
      });
      return res
        .status(422)
    ...
    const cleanPayload = {
      ...
      requirements: {
        ...
        required_specializations: Array.isArray(reqs.required_specializations)
          ...
          : undefined,
        preferred_skills: Array.isArray(reqs.preferred_skills)
      ...
      application_deadline: body.application_deadline
        ? parseDate(body.application_deadline)
        : undefined,
      notes: body.notes,
    };

    const task = await taskService.createForClinic(clinicId, cleanPayload, {
      reqId: req.reqId,
      actor: user.sub,
    });
    ...
    log.error({
      msg: "clinicTaskController.create error",
      error: err.message,
      stack: err.stack,
    });
    next(err);
...
const listForClinic = async (req, res, next) => {
  ...
  const user = req.user || {};
  log.info({
    msg: "clinicTaskController.listForClinic called",
    userId: user.sub,
    role: user.role,
    query: req.query,
  });
  ...
  try {
    ...
    if (q.priority) {
      if (!PRIORITY_ENUM.includes(q.priority))
        errors.push(`Invalid priority: ${q.priority}`);
    ...
    if (q.certification_level) {
      ...
      if (CERT_ALIASES[cert]) cert = CERT_ALIASES[cert];
      if (!CERT_ENUM.includes(cert))
        errors.push(`Invalid certification_level: ${q.certification_level}`);
      else filters["requirements.certification_level"] = cert;
    }

    if (q.specialization) {
      // exact match from enum list (schema will enforce)
      filters["requirements.required_specializations"] = String(
        q.specialization
      );
    }
    ...
    if (startFrom && isNaN(startFrom.getTime()))
      errors.push("start_from must be a valid date");
    if (startTo && isNaN(startTo.getTime()))
      errors.push("start_to must be a valid date");
    ...
    if (errors.length) {
      log.warn({
        msg: "clinicTaskController.listForClinic validation failed",
        errors,
      });
      return res
        .status(422)
    ...
    const result = await taskService.listForClinic(
      clinicId,
      {
        filters,
        page,
    ...
    log.error({
      msg: "clinicTaskController.listForClinic error",
      error: err.message,
      stack: err.stack,
    });
    next(err);
...
const getById = async (req, res, next) => {
  ...
  const { id } = req.params || {};
  log.info({
    msg: "clinicTaskController.getById called",
    userId: user.sub,
    role: user.role,
    id,
  });
  ...
  try {
    ...
    if (!isAdmin && !clinicId) {
      log.warn({
        msg: "No clinic scope on user for task fetch",
        userId: user.sub,
      });
      return res.status(401).json({ success: false, message: "Unauthorized" });
    }

    const task = await taskService.getByIdForClinic(
      id,
      { clinicId, isAdmin },
    ...
    if (!task) {
      log.info({
        msg: "Task not found or not in clinic scope",
        id,
        clinicId,
        isAdmin,
      });
    ...
    log.error({
      msg: "clinicTaskController.getById error",
      error: err.message,
      stack: err.stack,
    });
    next(err);
...
const updateById = async (req, res, next) => {
  ...
  const { id } = req.params || {};
  log.info({
    msg: "clinicTaskController.updateById called",
    userId: user.sub,
    role: user.role,
    id,
  });
  ...
  try {
    ...
    if (body.requirements) {
      ...
      if (r.minimum_experience != null) {
        ...
        if (Number.isNaN(me) || me < 0)
          errors.push("requirements.minimum_experience must be >= 0");
      ...
      if (r.required_specializations != null) {
        if (!Array.isArray(r.required_specializations)) {
          ...
          const bad = r.required_specializations.filter(
            (s) => !SPEC_ENUM.includes(s)
          );
          if (bad.length)
            errors.push(
              `requirements.required_specializations contains invalid values: ${bad.join(
                ", "
              )}`
            );
          else
            update.requirements.required_specializations =
              r.required_specializations;
        }
      }
      if (r.preferred_skills != null) {
        if (!Array.isArray(r.preferred_skills))
          errors.push("requirements.preferred_skills must be an array");
        else update.requirements.preferred_skills = r.preferred_skills;
      }
      // clean empty object
      if (Object.keys(update.requirements).length === 0)
    ...
    if (body.schedule) {
      ...
      if (s.break_duration_minutes != null) {
        ...
        if (Number.isNaN(breakMin) || breakMin < 0 || breakMin > 180) {
          errors.push(
            "schedule.break_duration_minutes must be between 0 and 180"
          );
        } else {
    ...
    if (body.compensation) {
      ...
      if (Object.keys(update.compensation).length === 0)
        delete update.compensation;
    }
    ...
    if (body.priority != null) {
      if (!PRIORITY_ENUM.includes(body.priority))
        errors.push(`priority must be one of: ${PRIORITY_ENUM.join(", ")}`);
      else update.priority = body.priority;
    }

    // location_details & other fields
    if (body.location_details != null)
      update.location_details = body.location_details;
    if (body.max_applications != null) {
      const ma = Number(body.max_applications);
      if (Number.isNaN(ma) || ma < 1 || ma > 1000)
        errors.push("max_applications must be 1..1000");
      else update.max_applications = ma;
    }
    if (body.auto_assign != null)
      update.auto_assign = Boolean(body.auto_assign);
    if (body.requires_background_check != null)
      update.requires_background_check = Boolean(
        body.requires_background_check
      );
    if (body.application_deadline != null) {
      const ad = parseDate(body.application_deadline);
      if (isNaN(ad.getTime()))
        errors.push("application_deadline must be a valid date");
    ...
    if (errors.length) {
      log.warn({
        msg: "clinicTaskController.updateById validation failed",
        errors,
      });
      return res
        .status(422)
    ...
    log.error({
      msg: "clinicTaskController.updateById error",
      error: err.message,
      stack: err.stack,
    });
    next(err);
...
const cancelById = async (req, res, next) => {
  ...
  log.info({
    msg: "clinicTaskController.cancelById called",
    userId: user.sub,
    taskId: id,
  });
  ...
  try {
    ...
    const result = await taskService.cancelForClinic(id, clinicId, reason, {
      reqId: req.reqId,
      actor: user.sub,
    });
    ...
    log.error({
      msg: "clinicTaskController.cancelById error",
      error: err.message,
      stack: err.stack,
    });
    next(err);
