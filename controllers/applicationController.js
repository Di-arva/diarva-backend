...
const listForTask = async (req, res, next) => {
  ...
  const { id } = req.params || {};
  log.info({
    msg: "applicationController.listForTask called",
    userId: user.sub,
    role: user.role,
    taskId: id,
  });
  ...
  try {
    ...
    const applications = await applicationService.listApplicationsForTask(
      id,
      clinicId
    );
    ...
    log.error({
      msg: "applicationController.listForTask error",
      error: err.message,
      stack: err.stack,
    });
    next(err);
...
const accept = async (req, res, next) => {
  ...
  const { id } = req.params || {};
  log.info({
    msg: "applicationController.accept called",
    userId: user.sub,
    role: user.role,
    applicationId: id,
  });
  ...
  try {
    ...
    log.error({
      msg: "applicationController.accept error",
      error: err.message,
      stack: err.stack,
    });
    next(err);
...
const reject = async (req, res, next) => {
  ...
  const { id } = req.params || {};
  log.info({
    msg: "applicationController.reject called",
    userId: user.sub,
    role: user.role,
    applicationId: id,
  });
  ...
  try {
    ...
    log.error({
      msg: "applicationController.reject error",
      error: err.message,
      stack: err.stack,
    });
    next(err);
...
const withdraw = async (req, res, next) => {
  ...
  const { id } = req.params || {};
  log.info({
    msg: "applicationController.withdraw called",
    userId: user.sub,
    role: user.role,
    applicationId: id,
  });
  ...
  try {
    ...
    log.error({
      msg: "applicationController.withdraw error",
      error: err.message,
      stack: err.stack,
    });
    next(err);
...
const discover = async (req, res, next) => {
  ...
  const user = req.user || {};
  log.info({
    msg: "assistantTaskController.discover called",
    userId: user.sub,
    q: req.query,
  });
  ...
  try {
    ...
    if (q.specialization) {
      if (!SPEC_ENUM.includes(q.specialization))
        errors.push(`Invalid specialization: ${q.specialization}`);
    ...
    if (errors.length) {
      log.warn({
        msg: "assistantTaskController.discover validation failed",
        errors,
      });
      return res
        .status(422)
    ...
    log.info({
      msg: "assistantTaskController.discover success",
      total: result.total,
      page: result.page,
      pages: result.pages,
    });
    ...
    log.error({
      msg: "assistantTaskController.discover error",
      error: err.message,
      stack: err.stack,
    });
    next(err);
...
const applyToTask = async (req, res, next) => {
  ...
  const { id } = req.params || {};
  log.info({
    msg: "assistantTaskController.applyToTask called",
    userId: user.sub,
    taskId: id,
  });
  ...
  try {
    ...
    const result = await applicationService.apply(user.sub, id, {
      reqId: req.reqId,
      actor: user.sub,
    });
    if (!result?.ok) {
      ...
      return res
        ...
        .json({
          success: false,
          message: result?.message || "Unable to apply",
        });
    }

    log.info({
      msg: "assistantTaskController.applyToTask success",
      taskId: id,
      applicationId: result.applicationId,
    });
    return res
      .status(201)
      .json({ success: true, data: { application_id: result.applicationId } });
  } catch (err) {
    log.error({
      msg: "assistantTaskController.applyToTask error",
      error: err.message,
      stack: err.stack,
    });
    next(err);
...
const getAllClinicApplicants = async (req, res, next) => {
  ...
  const user = req.user || {};
  log.info({
    msg: "applicationController.getAllClinicApplicants called",
    userId: user.sub,
  });
...
const getMyApplications = async (req, res, next) => {
  ...
  const user = req.user || {};
  log.info({
    msg: "applicationController.getMyApplications called",
    userId: user.sub,
  });
  ...
  try {
    ...
    if (
      q.status &&
      ["pending", "under_review", "accepted", "rejected", "withdrawn"].includes(
        q.status
      )
    ) {
    ...
    const applications = await mongoose
      ...
      .populate(
        "task_id",
        "title schedule compensation requirements status clinic_id"
      )
